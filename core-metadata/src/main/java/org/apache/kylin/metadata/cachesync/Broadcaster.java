...
public class Broadcaster {
            ...
            public void run() {
                ...
                while (true) {
                    try {
                        ...
                        final int thisRetryTime = broadcastEvent.getRetryTime() + 1;
                        broadcastEvent.setRetryTime(thisRetryTime);
                        if (thisRetryTime > retryLimitTimes) {
                            logger.warn("broadcastEvent retry up to limit times, broadcastEvent:{}, retry is: {}", broadcastEvent, thisRetryTime);
                            continue;
                        }
                        logger.info("Announcing new broadcast event: {}, retry is: {}", broadcastEvent, thisRetryTime);
                            ...
                            wipingCachePool.execute(new Runnable() {
                                ...
                                public void run() {
                                    try {
                                        ...
                                        logger.warn("Thread failed during wipe cache at {}, error msg: {}, base url is: {}, retry is: {}", broadcastEvent, e.getMessage(),
                                                restClient.getBaseUrl(), thisRetryTime);
                                        try {
                                            broadcastEvents.putLast(broadcastEvent);
                                        } catch (InterruptedException ex) {
                                            logger.warn("error retry failed broadcastEvent to queue, broadcastEvent:{}, error:{} ", broadcastEvent, ex);
                                        }
                                    }
                                }
                            });
    ...
    public void notifyListener(String entity, Event event, String cacheKey) throws IOException {
        ...
        if(entity.equals("project")){
            logger.info("entity:{},event:{},cacheKey:{},listener size:{}",entity,event,cacheKey,list.size());
        }
