...
public class JoinedFlatTable {
    ...
    public static String generateInsertDataStatement(IJoinedFlatTableDesc intermediateTableDesc, JobEngineConfig engineConfig, boolean redistribute, boolean forceJoin,KylinConfig kylinConfig) {
        StringBuilder sql = new StringBuilder();
        sql.append(generateHiveSetStatements(engineConfig));
        appendHiveOverrideProperties(kylinConfig,sql);
        sql.append("INSERT OVERWRITE TABLE " + intermediateTableDesc.getTableName() + " " + generateSelectDataStatement(intermediateTableDesc, redistribute, forceJoin) + ";").append("\n");
        return sql.toString();
    }
    ...
    public static String generateSelectDataStatement(IJoinedFlatTableDesc flatDesc, boolean redistribute, boolean forceJoin) {
        StringBuilder sql = new StringBuilder();
        sql.append("SELECT" + "\n");
        String tableAlias;
        ...
        appendJoinStatement(flatDesc, sql, tableAliasMap, forceJoin);
    ...
    private static void appendJoinStatement(IJoinedFlatTableDesc flatDesc, StringBuilder sql, Map<String, String> tableAliasMap, boolean forceJoin) {
                ...
                if (!dimTableCache.contains(dimTableName)) {
                    ...
                    if(forceJoin) {
                        for (int i = 0; i < pk.length; i++) {
                            if (flatDesc.getAllColumns().contains(fk[i])) {
                                needJoin = true;
                                break;
                            }
                        }
                    ...
                    if (cubeJoins.contains(join) || needJoin) {
    ...
    public static String generateRedistributeFlatTableStatement(IJoinedFlatTableDesc intermediateTableDesc) {
        ...
        if (distDcol != null) {
            ...
            CubeSegment segment = (CubeSegment)intermediateTableDesc.getSegment();
            RowKeyDesc rowKey = segment.getCubeDesc().getRowkey();
            RowKeyColDesc[] columns = rowKey.getRowKeyColumns();
            StringBuilder columnBuilder = new StringBuilder();
            int i = 0;
            for(RowKeyColDesc column : columns) {
                if(i>0) {
                    columnBuilder.append(",");
                }
                columnBuilder.append(colName(column.getColRef().getCanonicalName()));
                i++;
            }
            redistributeCol = columnBuilder.toString();
        }
        appendDistributeStatement(sql, redistributeCol);
        return sql.toString();
    }
