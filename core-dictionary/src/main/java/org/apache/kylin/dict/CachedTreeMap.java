...
//        this.versionDir = getLatestVersion(conf, fs, baseDir);
        this.versionDir = getLatestValidVersion(conf, fs, baseDir);
    ...
    //判断version文件夹下.index文件的大小，大小为0，表示该文件夹无效，则删除
    private static Path getLatestValidVersion(Configuration conf, FileSystem fs, Path baseDir) throws IOException {
        String[] versions = listAllVersions(fs, baseDir);
        if (versions.length > 0) {
            String validVersion = null;
            for(int i = versions.length -1;i>=0;i--){
                String version = versions[i];
                Path indexFile = new Path(version, ".index");
                FileStatus status = fs.getFileStatus(indexFile);
                if(status.getLen() > 0){
                    validVersion = version;
                    break;
                }else{
                    Path invalidVersionPath = new Path(version);
                    if(fs.exists(invalidVersionPath)){
                        logger.info("invalid version path should be deleted,{}",invalidVersionPath);
//                        fs.delete(invalidVersionPath,true);
                    }
                }
            }
            return new Path(validVersion);
        } else {
            // Old format, directly use base dir, convert to new format
            Path newVersionDir = new Path(baseDir, VERSION_PREFIX + System.currentTimeMillis());
            Path tmpNewVersionDir = new Path(baseDir, "tmp_" + VERSION_PREFIX + System.currentTimeMillis());
    ...
    public static FSDataInputStream openLatestIndexInput(Configuration conf, String baseDir) throws IOException {
        ...
        Path indexPath = new Path(getLatestValidVersion(conf, fs, basePath), ".index");
        return fs.open(indexPath, 8 * 1024 * 1024);
    }
