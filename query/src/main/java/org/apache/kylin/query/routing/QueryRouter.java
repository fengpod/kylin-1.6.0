...
public class QueryRouter {
    ...
    public static IRealization selectRealization(OLAPContext olapContext) throws NoRealizationFoundException {
        ...
        logger.debug("The project manager's reference is " + prjMgr);
        ...
        for (IRealization real : realizations) {
            if (real.isReady()) {
                if (real.getType() == RealizationType.CUBE && (real instanceof CubeInstance)) {
                    CubeInstance cube = (CubeInstance) real;
                    final KylinConfig kylinConfig = cube.getConfig();

                    boolean skip = false;
                    final Map<String, String> hiveConfOverride = kylinConfig.getHiveConfigOverride();
                    if (hiveConfOverride.isEmpty() == false) {
                        if(hiveConfOverride.keySet().contains("offline")) {
                            if (hiveConfOverride.get("offline").toLowerCase().equals("true")) {
                                skip = true;
                            }
                        }
                    }
                    ...
                    if (skip) {
                        logger.debug("cube:" + cube + " is offline, skip it.");
                        continue;
                    }
                }
        ...
        if(logger.isDebugEnabled()){
            logger.debug("Find candidates by table " + factTableName + " and project=" + projectName + " : " + StringUtils.join(candidates, ","));
        }
        ...
        if(logger.isDebugEnabled()){
            logger.debug("The realizations remaining: " + RoutingRule.getPrintableText(candidates) + " And the final chosen one is the first one");
        }
...
//                logger.debug("Adjust DimensionAsMeasure for " + functionDesc);
            }
        }
    }
