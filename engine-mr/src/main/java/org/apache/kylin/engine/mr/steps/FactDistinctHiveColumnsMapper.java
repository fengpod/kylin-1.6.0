...
public class FactDistinctHiveColumnsMapper<KEYIN> extends FactDistinctColumnsMapperBase<KEYIN, Object> {
    ...
    protected void setup(Context context) throws IOException {
        ...
        if (collectStatistics) {
            ...
            logger.info("baseCuboidId:"+baseCuboidId);
            logger.info("allCuboidsBitSetList size:"+allCuboidsBitSetList.size());
            logger.info("cuboidIdList size:"+cuboidIdList.size());
    ...
    private void putRowKeyToHLL(String[] row) {
        ...
        for (int i = 0; i < nRowKey; i++) {
            ...
            int idx = intermediateTableDesc.getRowKeyColumnIndexes()[i];
            if(idx == -1){
                RowKeyColDesc colDesc = cubeDesc.getRowkey().getRowKeyColumns()[i];
                logger.info("row key index error,col:{},colValues:{},rowKey:{}",colDesc.getColumn(),row,i);
            }
            String colValue = row[idx];
            if (colValue != null) {
