...
public class QueryUtil {
    ...
    public static String makeErrorMsgUserFriendly(Throwable e) {
        ...
        String ori = baos.toString();

        String ret = ori;
        String[] tokens = ori.split("\n");
        List<String> items = new LinkedList<String>();
        items.add(makeErrorMsgUserFriendly(msg.getMessage()));
        for (String token : tokens) {
            if (token.indexOf("Caused by") > -1 ) {
                items.add(token);
            }
        }

        if (items.size() > 0) {
            ret = StringUtils.join(items,"\n");
        }
