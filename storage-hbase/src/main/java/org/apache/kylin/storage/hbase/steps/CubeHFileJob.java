...
public class CubeHFileJob extends AbstractHadoopJob {
    ...
    public int run(String[] args) throws Exception {
        ...
        try {
            ...
            //针对此步单独设置内存
            overrideJobConfig(job.getConfiguration(),cube.getConfig());
    ...
    private static class KeyValueReducer extends KylinReducer<RowKeyWritable, KeyValue, ImmutableBytesWritable, KeyValue> {
        ...
        @Override
        public void reduce(RowKeyWritable row, Iterable<KeyValue> kvs, Context context) throws java.io.IOException, InterruptedException {
            for (KeyValue kv : kvs) {
    ...
    private void overrideJobConfig(Configuration configuration, KylinConfig config) {
        Map<String,String> memOpts = config.getKylinJobHfileMemConf();
        for (Entry<String, String> entry : memOpts.entrySet()) {
