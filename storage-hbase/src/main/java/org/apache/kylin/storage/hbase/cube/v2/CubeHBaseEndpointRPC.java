...
//        logger.debug("Create GTScanner");
        ...
        setStorageScanRowNumThreshold(scanRequest);
...
//        logger.info("Submitting rpc to "+shardNum+" shards starting from shard "+cuboidBaseShard+", scan range count "+rawScans.size());
        ...
        logger.debug("begin query hbase...");
            ...
            executorService.submit(new Runnable() {
                ...
                public void run() {
                        ...
                        table.coprocessorService(CubeVisitService.class, startKey, endKey, //
                                new Batch.Call<CubeVisitService, CubeVisitResponse>() {
                                    ...
                                    public void update(byte[] region, byte[] row, CubeVisitResponse result) {
                                        logger.debug(queryId+" query hbase end...");
                                        if (region == null)
                                            return;
                    ...
                    if (abnormalFinish[0]) {
                        ...
                        logger.error(logHeader + "Error when visiting cubes by endpoint,scan threshold:{}",scanRequest.getStorageScanRowNumThreshold(), ex); // double log coz the query thread may already timeout
                        epResultItr.notifyCoprocException(ex);
                        return;
                    }
                }
        ...
        return new StorageResponseGTScatter(fullGTInfo, epResultItr, scanRequest.getColumns(), totalScannedCount.get(),totalScannedCount, scanRequest.getStoragePushDownLimit());
    }

    private void setStorageScanRowNumThreshold(GTScanRequest scanRequest) {
        int configThreshold = cubeSeg.getConfig().getCubeStorageScanRowNumThreshold();
        int scanNumThreshold = scanRequest.getStorageScanRowNumThreshold();
        scanRequest.setStorageScanRowNumThreshold(Math.max(configThreshold, scanNumThreshold));
    }
