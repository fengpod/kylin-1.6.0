...
//            context.increaseTotalScanCount(iterator.scanner.getScannedRowCount());
            iterator.close();
        }
    }
    ...
    private void flushScanCountDelta() {
        long curScanCount = 0;
        for(CubeSegmentScanner scanner : scanners){
            curScanCount = curScanCount + scanner.getScannedRowCount();
        }
        context.increaseTotalScanCount(curScanCount - preScanCount);
        preScanCount = curScanCount;
        scanCountDelta = 0;
    }
